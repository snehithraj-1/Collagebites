...
export default function ClgDashboardPage() {
  const { profile, logout } = useClgAuth();

  const [orders, setOrders] = useState([]);
  const [restaurant, setRestaurant] = useState({ id: 'clg-bites-biryani-nation', name: 'CLG Bites Biryani Nation', is_open: true });
  ...
  // 1. Fetch CLG Bites Status
  const loadRestaurantStatus = useCallback(async () => {
    try {
      const res = await fetch('/api/restaurants');
      if (res.ok) {
        const json = await res.json();
        const found = (json.restaurants || []).find(r => r.id === 'clg-bites-biryani-nation');
        if (found) setRestaurant(found);
      }
    } catch (e) {}
  }, []);

  // 2. Fetch CLG Bites Delivery Partners Only
  const loadDeliveryPartners = useCallback(async () => {
    try {
      const res = await fetch('/api/delivery-partners?restaurant_id=clg-bites-biryani-nation');
      if (res.ok) {
        const json = await res.json();
  ...
  // 3. Fetch CLG Bites Orders Only with Live Audio Alert
  const loadOrders = useCallback(async (silent = false) => {
    if (!silent) setIsRefreshing(true);
    try {
      const res = await fetch('/api/orders?restaurant_id=clg-bites-biryani-nation');
      if (res.ok) {
        const json = await res.json();
      ...
      console.warn('[CLG Orders Error]:', err);
    } finally {
  ...
  const handleToggleOpen = async () => {
    ...
    try {
      const res = await fetch('/api/restaurants/clg-bites-biryani-nation/toggle', {
        method: 'POST',
    ...
    <div className="min-h-screen bg-[#080E1A] text-slate-100 pb-20 font-sans">
      ...
      <header className="sticky top-0 z-40 bg-[#0F172A]/95 backdrop-blur-md border-b border-slate-800 shadow-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-2xl bg-gradient-to-tr from-amber-500 to-yellow-600 flex items-center justify-center text-white text-2xl shadow-lg shadow-amber-500/20 border border-amber-400/30">
              🍗
            </div>
            <div>
              ...
              <div className="text-[11px] text-slate-400 flex items-center gap-1.5">
                <span>Staff: <strong className="text-slate-200">{profile?.name || 'CLG Bites Staff'}</strong></span>
                <span>•</span>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-xl flex items-center justify-between">
            <div>
              <div className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">CLG Bites Orders</div>
              <div className="text-2xl font-black text-amber-400 font-['Outfit'] mt-1">
                {orders.length}
              </div>
            </div>
            <div className="w-12 h-12 rounded-2xl bg-amber-500/10 text-amber-400 flex items-center justify-center text-2xl">
              🍗
            </div>
          </div>
        ...
        {/* CLG Bites Orders Table (with Quick Dispatch & Deliver buttons) */}
        <OrdersTable
          orders={orders}
      ...
      {/* Delivery Partners Management Modal (Isolated to CLG Bites) */}
